...
아이폰 페이스 트래킹(iFacialMocap / FaceMotion3D)을 녹화해뒀다가, 아이폰 없이도
[Warudo](https://warudo.app)로 재생 송출하는 상시 실행 UDP 프록시입니다.
...
```
[패스스루 / 녹화]
아이폰 앱 --UDP--> ARKit Recorder :49983 --그대로 전달--> Warudo :49984
                        |
                        +--> (녹화 중) clips/이름.jsonl 무손실 기록

[재생 / 스크럽]
클립 --> ARKit Recorder --원래 타이밍대로--> Warudo :49984
         (이 동안 라이브 패킷은 차단, 종료 시 크로스페이드 복귀)
```

Warudo의 iFacialMocap Receiver는 순수 UDP 수신이라 보내는 쪽이 아이폰인지
이 프로그램인지 구분하지 않습니다. 패킷을 원본 그대로 저장하므로 재생 충실도는
100%입니다.

## 기능

- **패스스루**: 평상시엔 투명한 프록시 (아이폰 → Warudo)
...
- **타임라인**: 활동량·블렌드쉐이프(52종) 곡선, 스크럽(드래그하면 아바타가 실시간으로
  따라옴), 비파괴 트리밍
- **일시정지**: 타임라인 클릭/스크럽 릴리즈 = 해당 프레임 고정 (킵얼라이브로 Warudo
...
Warudo 씬의 **iFacialMocap Receiver 포트를 49983 → 49984로 변경**하세요.
아이폰 앱 설정(PC IP 입력)은 그대로 둡니다.
...
프로그램이 꺼져 있으면 트래킹이 Warudo에 전달되지 않으므로 방송 중 상시 실행이
전제입니다.
...
- iFacialMocap 프로토콜을 쓰는 트래킹 앱 (iFacialMocap, FaceMotion3D 등)
